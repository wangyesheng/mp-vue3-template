<template>
  <view
    ...
    <canvas
      ...
      id="offscreen"
      :style="
        'width:' + offscreenSize[0] + 'px;height:' + offscreenSize[1] + 'px'
      "
      :width="offscreenSize[0]"
      :height="offscreenSize[1]"></canvas>
    <view
      v-if="showMask"
      class="mask"
      @touchstart="touchStart"
    ...
    <view
      ...
      :rempty="rempty"
      :change:rempty="sign.isEmpty"></view>
    <!-- #endif -->
    <!-- #ifdef APP-NVUE -->
    <web-view
...
export default {
  ...
  computed: {
    ...
    offscreenSize() {
      ...
      return this.landscape
        ? [offscreenHeight, offscreenWidth]
  ...
  async mounted() {
    ...
    this.offscreenHeight = this.canvasHeight =
      this.signature.canvas.get('height')
    ...
    canvasToTempFilePath(param = {}) {
      ...
      const canvasToTempFilePath = async (image) => {
        const createCanvasContext = () => {
          const useOffscreen =
            useCanvas2d && !!uni.createOffscreenCanvas && preferToDataURL
      ...
      const next = async () => {
        ...
        // #endif
        toDataURL(this.canvasId, this, param)
