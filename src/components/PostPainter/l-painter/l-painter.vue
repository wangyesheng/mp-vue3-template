...
export default {
  ...
  methods: {
    ...
    async render(args = {}) {
      ...
      const { use2dCanvas, boardWidth, boardHeight, canvas, afterDelay } = this
      ...
      try {
        ...
        const { width, height } = await this.painter.source(
    ...
    canvasToTempFilePath(args = {}) {
      return new Promise(async (resolve, reject) => {
        ...
        const success = async (res) => {
          try {
            const tempFilePath = await this.setFilePath(
              res.tempFilePath || res,
              args
            )
        ...
        const { top: y = 0, left: x = 0, width, height } = this.boundary || this
