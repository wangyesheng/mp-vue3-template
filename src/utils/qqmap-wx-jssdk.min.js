  ...
  handleData(param, data, feature) {
    if (feature === 'search') {
      ...
      if (reverseGeocoderResult.pois) {
        const pois = reverseGeocoderResult.pois
        const poisSimplify = []
      ...
      const geocoderResult = data.result
      const geocoderSimplify = {
      ...
      const provinceResult = data.result[0]
      const cityResult = data.result[1]
      const districtResult = data.result[2]
      param.success(data, {
        provinceResult,
        cityResult,
        districtResult
      ...
      const districtByCity = data.result[0]
...
class QQMapWX {
  ...
  getCityList(options) {
    ...
    options = options || {}
    Utils.polyfillParam(options)
    const requestParam = { output: 'json', key: that.key }
    wx.request(
      Utils.buildWxRequestConfig(
        options,
  ...
  getDistrictByCityId(options) {
    ...
    const requestParam = { id: options.id || '', output: 'json', key: that.key }
    wx.request(
      Utils.buildWxRequestConfig(
        options,
  ...
  calculateDistance(options) {
    ...
    if (requestParam.mode == 'straight') {
      var locationsuccess = function (result) {
        const locationTo = Utils.getEndLocation(requestParam.to)
        const data = {
          message: 'query ok',
          result: { elements: [] },
          status: 0
        ...
        const calculateResult = data.result.elements
